 ...
 uint8_t OPRMode = NDOF; //IMU;      			// specify operation mode for sensors
...
void readQuat(){
 ...
 float gradiGiroscopio(int asse){
	 if(asse==asseZ){
		gradi_=readAxis(asse)-correzzione_asseZ;
		if(gradi_<0)gradi_+=360;
		else if(gradi_>=360) gradi_-=360;
	 }else if(asse==asseY) gradi_=readAxis(asse)-init_asseY;
	 else readAxis(asse);
	 return gradi_;
 }